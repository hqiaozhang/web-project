...
define(function (require) {
  ...
  var doubleArea = {
    ...
    drawCharts: function (id, dataset, opt) {
      ...
      // 文字组图层  
      var textGroup = d3.select(id).select('svg').selectAll('.text-group')
      if (textGroup.node()) {
        textGroup = d3.select(id).select('svg').select('.text-group')
      } else {
    ...
    addTopText: function (text, xScale, yScale, cfg) {
      text.attr('class', cfg.class)
        ...
        .attr('y', function (d) {
          if (cfg.type === 1) {
            return yScale(d.access) + 10
          }
          return yScale(d.leave) + 10
        })
        .text(function (d) {
          if (cfg.type === 1) {
            return d.access
