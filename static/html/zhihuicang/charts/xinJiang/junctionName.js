...
define(function (require) {
  ...
  var areaCharts = {
    ...
    drawCharts: function (id, data1, opt) {
      ...
      var y = height - config.grid.y - 200
    ...
    addXAxis: function (svg, config, xData) {
      ...
      var xAxis = d3.svg.axis()
        ...
        .tickFormat(function (d, i) {
          return d.length > 3 ? d.substr(0, 3) + '...' : d
        })
      ...
      var setTime
      svg.selectAll('.axis-x text').on('mouseover', function (d) {
        var top = 480
        var left = xScale(d)+40
        clearTimeout(setTime)
        
        if ($('.tool-tip').length < 1) {
          $('.junctionName').append('<div class="tool-tip"><span></span></div>')
        }
        $('.tool-tip').css({
            top: top,
            left: left
