...
define(function(require) {
  ...
  var map = {
    ...
    init: function(masterFile, data, type, mapurl, caseNumberUrl){
      ...
      _common.mapInit(masterFile, data, config, type, '#detectiveCaseMap', mapurl, caseNumberUrl, '破案数')    
    },
    ...
    renderTotal: function(data){
       var ratio = Math.floor(data.total * 100) + '%'
      $('.count-num').html(ratio)
    },
    ...
    victory: function(data){
      _common.victory(data, config, '#detectiveCaseMap', '#detectiveCaseVictory', 0)         
    }
